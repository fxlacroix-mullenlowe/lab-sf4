<?php

namespace App\Repository\Analyzer;

use App\Entity\Analyzer;
use Doctrine\Bundle\DoctrineBundle\Repository\ServiceEntityRepository;
use Symfony\Bridge\Doctrine\RegistryInterface;

/**
 * @method Analyzer|null find($id, $lockMode = null, $lockVersion = null)
 * @method Analyzer|null findOneBy(array $criteria, array $orderBy = null)
 * @method Analyzer[]    findAll()
 * @method Analyzer[]    findBy(array $criteria, array $orderBy = null, $limit = null, $offset = null)
 */
class AnalyzerRepository extends ServiceEntityRepository
{
    public function __construct(RegistryInterface $registry)
    {
        parent::__construct($registry, Analyzer::class);
    }

//    /**
//     * @return Analyzer[] Returns an array of Analyzer objects
//     */
    /*
    public function findByExampleField($value)
    {
        return $this->createQueryBuilder('a')
            ->andWhere('a.exampleField = :val')
            ->setParameter('val', $value)
            ->orderBy('a.id', 'ASC')
            ->setMaxResults(10)
            ->getQuery()
            ->getResult()
        ;
    }
    */

    /*
    public function findOneBySomeField($value): ?Analyzer
    {
        return $this->createQueryBuilder('a')
            ->andWhere('a.exampleField = :val')
            ->setParameter('val', $value)
            ->getQuery()
            ->getOneOrNullResult()
        ;
    }
    */
}
